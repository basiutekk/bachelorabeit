...
if __name__ == '__main__':
    ...
    while numgenerated < num_graph:
        ...
        # g_list.append(g)
        with open('%s/gtype-%s-nrange-%d-%d-n_graph-%d-p-%.3f-m-%d.pkl' % (save_dir, graph_type, min_n, max_n, num_graph, p, m), 'ab') as fout:
            cp.dump(g, fout,protocol=2)
       
        
        with open('%s/gtype-%s-nrange-%d-%d-n_graph-%d-p-%.3f-m-%d.pkl' % (save_dir, graph_type, min_n, max_n, num_graph, p, m), 'rb') as f:
            for i in range(numgenerated):
                cp.load(f)
            graph_loaded=cp.load(f)
            print("loaded eges:",nx.number_of_edges(graph_loaded))
        numgenerated += 1
